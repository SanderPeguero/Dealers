...
import {uploadBackgroundImage, AddNewBackgroundImage } from '../../Functions/HomeAdmin/HomeAdmin';
import { useContextCar } from '../../Context/Context';
import { useState } from 'react';


function AddBackgroundImage({openAddImageModal, HandleClose}) {
const {setCheckContact} = useContextCar()
    ...
    const handleSaveImage = async () => {
        ...
        if (selectedImage) {
            const fileUrl = await uploadBackgroundImage(selectedImage);
            if (fileUrl) {
                const NewBackgroundImage = {
                    UrlBackgroundImage: fileUrl
                };
                await AddNewBackgroundImage(NewBackgroundImage);
    ...
    if (!openAddImageModal) return null;
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 w-full backdrop-blur-sm">
            <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-96 text-white">
                <h2 className="text-xl font-semibold mb-4">Agregar Imagen de Fondo</h2>
                <div className="space-y-7">
                    <div className='mb-4'>
                        <label className='text-sm'>Sube tu imagen de fondo de Contacto</label>
